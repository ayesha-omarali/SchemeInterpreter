...
(begin (print 'hello) '(2 3 4 5) (* 3 4))
; expect 12

(begin (print 'aloha) (+ (- 500 10) 909))
; expect 1399

;Question A8
(lambda (x yy zzz) (print x) (* yy zzz))
; expect (lambda (x yy zzz) (begin (print x) (* yy zzz)))

(lambda (k (* 4 i)) (* (+ k i) 3))
; expect Error

(lambda (a b c d e f g h i j k l m n o p) (a * e * z))
; expect (lambda (a b c d e f g h i j k l m n o p) (a * e * z))

;Question A9
(define (f x z) (* z x))
; expect f

(define (g y) (* 3 (+ 4 (/ 700 0))))
; expect g

(define (element p) (/ 1 0))
; expect element
...
(define (fn x y y 1) (print 'passed))
; expect Error

;Question A13
(if (= 1 1) (print 'passed) (/ 1 0))
; expect okay

(if (= (+ 3 6) (- 27 30)) (print 'finished!) (print 'jk))
; expect okay
...
;Question 18
(cons-stream 4 (cons-stream 6 (cons-stream 8 (cons-stream 10 the-empty-stream))))
; expect (4 . #[promise (not forced)])

(cons-stream (+ 3 (- 6 4)) (cons-stream 'x (cons-stream 'y the-empty-stream)))
; expect (5 . #[promise (not forced)])

(cons-stream (define (f x) (* 3 x)) (cons-stream 3 nil))
; expect (f . #[promise (not forced)])

;Question 19
(define x (cons-stream 3 (cons-stream 4 (cons-stream 5))))
; expect x

(stream-car x)
; expect 3

(define y (cons-stream (define (f x) x + 2) cons-stream(3 nil)))
; expect Error: too many operands in form

(define new-f (stream-car y))
; expect Error: unknown identifier: y

(define joop (cons-stream 'soup (cons-stream 'moop (cons-stream ((/1 0) nil)))))
; expect joop
