...
(define (patterns)
  ; YOUR CODE HERE
  (define (over x)
      (append x (cons 'over nil)))

  (define (under x)
      (append x (cons 'under nil)))

  (define helper_stream (cons-stream '()
      (interleave (stream-map over helper_stream) (stream-map under helper_stream))))

  (define (predicate lst)
     (and (member? 'over lst) (member? 'under lst)))      

  (stream-filter predicate helper_stream))
